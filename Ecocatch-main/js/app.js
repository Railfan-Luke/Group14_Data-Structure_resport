      ...
      navigator.mediaDevices.getUserMedia({ video: { facingMode: "environment" } })
        .then(stream => {
          ...
          btnCamera.onclick = () => {
            ...
            yoloStatusEl.textContent = "?? YOLOv8 Ëæ®Ë?‰∏≠‚Ä?;
              ...
              fetch("http://192.168.11.147:8000/detect", {
                ...
                .then(data => {
                  ...
                  yoloStatusEl.innerHTML = `
                    <span class="text-secondary font-bold">YOLOv8 Ëæ®Ë?ÁµêÊ?Ôº?/span>
                    <span class="text-white font-bold">${label}</span>
                    <span class="text-primary ml-2">‰ø°Â?Â∫?${pct}%</span>
                  `;
                ...
                .catch(err => {
                  console.warn("YOLOv8 API ???Â§±Ê?ÔºåÊé°?®Ê®°?¨Ëæ®Ë≠òÔ?", err);
                  yoloStatusEl.innerHTML = `<span class="text-error">??YOLOv8 API ?°Â??âÔ??°Áî®Ê®°Êì¨Ëæ®Ë?</span>`;
        ...
        .catch(err => {
          alert("?°Ê??üÂ??∏Ê?Ê¨äÈ?ÔºåÊé°?®Ê®°?¨Ê??ß„Ä?);
  ...
  if (currentDecisionNode.isLeaf) {
    ...
    qnaContainer.innerHTML = `
      <div class="glass-panel p-md rounded-xl text-center space-y-md">
        <h4 class="font-title-md text-primary">?å≥ Ê±∫Á?Ê®πËµ∞Ë®™Â??¢Ô?ÁµêÊ??∫Ô?${currentDecisionNode.label}</h4>
        <button id="btn-qna-apply" class="bg-primary text-on-primary px-4 py-2 rounded-xl text-label-md font-bold w-full">ËºâÂÖ• HashMap Ë≥áÊ?</button>
      </div>
    `;
  ...
  qnaContainer.innerHTML = `
    <div class="glass-panel p-md rounded-xl space-y-md">
      <div class="flex items-center justify-between">
        <span class="text-xs text-on-surface-variant font-bold uppercase tracking-wider">?å≥ Ê±∫Á?Ê®πÁ?ÈªûÔ?${currentDecisionNode.label}</span>
...
function setupMap() {
  ...
  userMarker.bindPopup("<b>‰Ω†Á?‰ΩçÁΩÆ</b><br>?ØÊ??≥‰ª•?πË?Â∫ßÊ?").openPopup();
  ...
  recycleStations.forEach(station => {
    ...
    marker.bindPopup(`<b>${station.name}</b><br>?∂Ô?${station.accepted.map(id => hashMap.get(id)?.name || id).join(", ")}`);
  ...
  btnLocateMe?.addEventListener("click", () => {
    ...
    navigator.geolocation.getCurrentPosition(
      (position) => {
        ...
        userMarker.bindPopup("<b>‰Ω†Á?‰ΩçÁΩÆ</b><br>Â∑≤Â?‰ΩçÂ??êÔ??ØÊ??≥‰ª•ÂæÆË™øÂ∫ßÊ?").openPopup();
      ...
      (error) => {
        console.error("ÂÆö‰?Â§±Ê?Ôº?, error);
        let msg = "?°Ê??ñÂ??®Á?ÂÆö‰?‰ΩçÁΩÆ??;
        switch (error.code) {
          case error.PERMISSION_DENIED:
            msg = "ÂÆö‰?Â§±Ê?ÔºöË??ÅË®±?èË¶Ω?®Â??ñÊÇ®?Ñ‰?ÁΩÆÊ??ê„Ä?;
            break;
          case error.POSITION_UNAVAILABLE:
            msg = "ÂÆö‰?Â§±Ê?Ôºö‰?ÁΩÆË?Ë®äÁÑ°Ê≥ïÂ?ÂæóÔ?Ë´ãÁ¢∫Ë™?GPS Â∑≤È??ü„Ä?;
            break;
          case error.TIMEOUT:
            msg = "ÂÆö‰?Â§±Ê?ÔºöÂ?Âæó‰?ÁΩÆË??ÇÔ?Ë´ãÈ?Ë©¶„Ä?;
            break;
        }
        alert(msg);
...
function searchNearestBin(categoryId = null) {
  ...
  if (candidates.length === 0) {
    alert("?ÆÂ?Ê≤íÊ??ûÊî∂ÈªûÊîØ?¥Ë©≤?ÉÂúæÁ®ÆÈ??ûÊî∂Ôº?);
    return;
  }
    ...
    debugPanel.innerHTML = `
      <div class="flex items-center gap-2 text-xs">
        ...
        <span class="font-label-sm text-on-surface-variant">
          KD-Tree ?úÂ?ÂÆåÊ?ÔºöÂ? <span class="text-primary">${candidates.length}</span> ?ãÂÄôÈÅ∏ÈªûÊâæ?∞Ê?ËøëÈ?ÔºåËÄóÊ? <span class="text-primary">12ms</span>
        </span>
      </div>
      <div class="h-4 w-[1px] bg-white/10"></div>
      <div class="flex items-center gap-2 text-xs">
        <span class="material-symbols-outlined text-secondary text-sm">reorder</span>
        <span class="font-label-sm text-on-surface-variant">Dijkstra ?ÜÁ??™Â?‰ΩáÂ?‰ΩúÂ?‰∏?/span>
      </div>
    `;
  }
  ...
  if (kdTraceList) {
    ...
    searchResult.history.forEach((step, idx) => {
      ...
      li.innerHTML = `
        <span>Step ${idx+1}: Ëµ∞Ë®™ [${step.nodeName}]</span>
        <span class="text-secondary">?áÂ?Á∂≠Â∫¶: ${step.axis} | Ë∑ùÈõ¢: ${step.dist}</span>
      `;
  ...
  if (dijkstraTraceList && routeResult.path.length > 0) {
    ...
    divPath.innerHTML = `?? Dijkstra ?Ä?Ä?≠Ë∑ØÂæ? ${pathStr}`;
    ...
    divVisited.innerHTML = `?? ?ÄÂ∞èÂ?Á©?(Min-Heap) ?íÂ?Ëµ∞Ë®™ÁØÄÈªûÈ?Â∫? <br> ${routeResult.visitedOrder.map(id => graphNodes[id].name || id).join(" -> ")}`;
...
function handleDepositSuccess() {
  ...
  btn.innerHTML = '<span class="material-symbols-outlined animate-spin text-sm">sync</span> ?∫ÊÖßÈ©óË?‰∏?..';
  btn.disabled = true;
  ...
  setTimeout(() => {
    ...
    // Success styling
    btn.innerHTML = `<span class="material-symbols-outlined">stars</span> ?êÂ??ïÈ?Ôº?${earnedPts} Ê∏õÁ¢≥Èªû`;
    ...
    // Add activity record
    addActivityRecord(activeScanItem ? activeScanItem.name : "?ûÊî∂?©Â?", earnedPts);

    // Alert
    setTimeout(() => {
      alert(`?? ?≠Â?ÂÆåÊ??ÜÈ??ïÈ?ÔºÅ\n?≤Â?ÈªûÊï∏: +${earnedPts} pts\nÊ∏õÁ¢≥?? -${co2Saved} kg`);
...
function addActivityRecord(itemName, points) {
  ...
  item.innerHTML = `
    <div class="flex items-center gap-md">
      ...
      <div>
        <p class="font-body-md text-body-md text-on-surface">?ïÈ? ${itemName}</p>
        <p class="font-label-sm text-label-sm text-on-surface-variant/60">?∞‰??ûÊî∂Á´????õÂ?</p>
      </div>
    </div>
    ...
    row.innerHTML = `
      <div class="flex items-center gap-4">
        ...
        <div class="flex items-center gap-3">
          ...
          <div>
            <p class="font-title-md text-on-surface text-sm ${isCurrentUser ? 'font-bold text-primary' : ''}">${u.name} ${isCurrentUser ? '(‰Ω?' : ''}</p>
            <p class="text-xs text-on-surface-variant/70">${u.level || '?∞Á?Ë°õÂ£´'}</p>
          </div>
        </div>
      </div>
...
function redeemCoupon(couponId) {
  ...
  if (userProfile.points < coupon.points) {
    alert(`??ÈªûÊï∏‰∏çË∂≥ÔºÅ\n?®ÁõÆ?çÊ? ${userProfile.points} ÈªûÔ??åÊ?Ê≠§Â??ÅÈ?Ë¶?${coupon.points} Èªû„ÄÇ`);
    return;
  }

  const confirmRedeem = confirm(`Á¢∫Â?Ë¶ÅËä±Ë≤?${coupon.points} ÈªûÂ??õ„Ä?{coupon.merchant} - ${coupon.title}?çÂ?Ôºü`);
  ...
  alert(`?? ?åÊ??êÂ?ÔºÅ\nÂ∑≤Êâ£?§È??? ${coupon.points} pts\n?©È?ÈªûÊï∏: ${userProfile.points} pts\n?™Ê??∏Ê?Á¢ºÂ∑≤?ºÈÄÅËá≥?®Á??êÂ∞±Â§æ‰∏≠ÔºÅ`);
}
...
function setupLogin() {
  ...
  const performLogin = (username) => {
    ...
    loginSubmitBtn.innerHTML = `<span class="material-symbols-outlined text-lg animate-spin">sync</span> È©óË?‰∏?..`;
    
    setTimeout(() => {
      if (username && username.trim() !== "") {
        let displayName = username.split('@')[0];
        displayName = displayName.charAt(0).toUpperCase() + displayName.slice(1);
        userProfile.name = `${displayName} (‰Ω?`;
      ...
      alert(`Ê≠°Ë??û‰?Ôº?{userProfile.name}ÔºÅ\nÂ∑≤Ê??üÁôª??EcoCatch ?∫ÊÖß?ÉÂúæ?ÜÈ?Âπ≥Âè∞?Ç`);
    }, 1200);
  };
